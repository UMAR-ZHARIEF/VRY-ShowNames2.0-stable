...
class AccountAuth:
    ...
    def get_current_version(self):
        version = static_content.get_json("/v1/version")
        if version is None:
            raise ConnectionError("valorant-api.com /v1/version unavailable")
        return version["data"]
    ...
    def get_account_data(self):
        ...
        contracts = static_content.get_json("/v1/contracts")
        if contracts is None:
            raise ConnectionError("valorant-api.com /v1/contracts unavailable")
        contracts = [a for a in contracts["data"] if a["content"]["relationType"] == "Season"]
        bp = contracts[-1]
