...
class Colors:
    ...
    def get_perf_bonus_color(self, value_str):
        if value_str == "N/A":
            return color("N/A", fore=(46, 46, 46))
        try:
            avg = float(value_str.split(" ")[0])
        except (ValueError, IndexError):
            return value_str
        if avg >= 9.0:
            return color(value_str, fore=(241, 39, 39))   # Red
        elif avg >= 5.0:
            return color(value_str, fore=(207, 207, 76))  # Yellow
        else:
            return color(value_str, fore=(255, 255, 255)) # White
