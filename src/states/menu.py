    ...
    def get_party_members(self, self_puuid, presencesDICT):
        ...
        for presence in presencesDICT:
            if presence["puuid"] == self_puuid:
                # If multiple XMPP resources are online for the same
                # account (e.g. official Riot Client + V3 probe session),
                # the presence dict contains one entry per resource.
                # De-dup so the user appears only once.
                if self_puuid in seen_puuids:
                    continue
                seen_puuids.add(self_puuid)
                decodedPresence = self.presences.decode_presence(presence["private"])
                if decodedPresence["isValid"]:
        ...
        for presence in presencesDICT:
            if presence["puuid"] == self_puuid:
                continue # Skip self
            # De-dup by puuid (other party members may also be online
            # on multiple XMPP resources).
            if presence["puuid"] in seen_puuids:
                continue
            seen_puuids.add(presence["puuid"])
