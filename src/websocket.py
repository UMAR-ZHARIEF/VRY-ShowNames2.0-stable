...
class Ws:
    def __init__(self, lockfile, Requests, cfg, colors, hide_names, server, rpc=None, name_cache=None):
    ...
    async def recconect_to_websocket(self, initial_game_state):
        ...
        for attempt in range(max_retries):
            try:
                async with websockets.connect(url, ssl=self.ssl_context, additional_headers=local_headers) as websocket:
                    await websocket.send('[5, "OnJsonApiEvent_chat_v4_presences"]')
                    # Always subscribe to chat messages so we can cache real names
                    # (even if game_chat display flag is off)
    ...
    def handle(self, m, initial_game_state):
        ...
        if resp_json[2].get("uri") == "/chat/v4/presences":
            ...
            msg_game_name = message.get('game_name', '')
            msg_game_tag = message.get('game_tag', '')

            # Always cache the name from chat (bypasses incognito)
            if self.name_cache and msg_puuid and msg_game_name:
                cached_name = f"{msg_game_name}#{msg_game_tag}" if msg_game_tag else msg_game_name
                self.name_cache.set(msg_puuid, cached_name, source="chat-message")
                self.name_cache.save_now()

            # Display chat messages only if feature flag enabled
            if not self.cfg.get_feature_flag("game_chat"):
                return None
            ...
            if "ares-coregame" in message.get("cid", "") and message.get("id") not in self.id_seen:
                ...
                name = f"{msg_game_name}#{msg_game_tag}"
